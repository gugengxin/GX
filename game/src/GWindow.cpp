...
#if defined(GX_METAL)
void* GWindow::Canvas::metalNewRCE()
{
    return m_Window->getContext().newRenderCommandEncoder();
}
...
- (id)initWithDelegate:(GWindow*)dge frame:(CGRect)rc
{
    ...
    if (self) {
        ...
        _metalLayer.pixelFormat     = GX_CAST_S(MTLPixelFormat, GMTLContext::layerPixelFormat());
